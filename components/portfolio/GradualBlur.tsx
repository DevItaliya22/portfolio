...
const POSITION_DIR: Record<Position, string> = {
  top: 'to top',
  bottom: 'to bottom',
  left: 'to left',
  right: 'to right',
...
const getGradientDirection = (position: Position): string =>
  POSITION_DIR[position] ?? 'to bottom';
...
interface GradualBlurProps extends Record<string, unknown> {
...
function GradualBlur(props: GradualBlurProps) {
  ...
  const config = useMemo(() => {
    const presetConfig =
      props.preset && props.preset in PRESETS
        ? PRESETS[props.preset as keyof typeof PRESETS]
        : {};
    return mergeConfigs(DEFAULT_CONFIG, presetConfig, props) as Record<
      string,
      unknown
    > & {
      position: Position;
  ...
  const containerStyle = useMemo((): React.CSSProperties => {
    ...
    if (isVertical) {
      ...
      baseStyle.width =
        (responsiveWidth as string) || (responsiveHeight as string);
